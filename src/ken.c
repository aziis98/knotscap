...
#define MAX_COMPONENTS 1	/* number of components of complex 
				   should be simply connected here */
...
struct K_data
 {
	...
	int flower[NUM_PETALS+1];	/* list of nodes in flower of this node,
...
/* -------- read data phase ----------------- */
	...
	if (!complex_count(&packdata) || !facedraworder(&packdata,NULL))
	 {
		fprintf(stderr,"Failed due to combinatoric problems.");
		exit(2);
	 }
...
readpack() /* read pack "knot-comb.p". Return 1 if okay. */
{
		...
		&& (strcmp(command,"END")!=0) )
	 {
		...
		if (strcmp(command,"ALPHA/BETA/GAMMA:")==0)
		...
		else if (strcmp(command,"FLOWERS:")==0)
		 {
		      for (i=1;i<=count;i++)
		       {
		         if (!fscanf(fp,"%d",&k)) return 0;
			 if ( !fscanf(fp,"%d",&pK_ptr[k].num)
			    || pK_ptr[k].num>NUM_PETALS ) 
				return 0;
	...
	for (i=1;i<=count;i++)
		h_anglesum(&packdata,i,pR_ptr[i].rad,
			&(pR_ptr[i].curv));
	return (1);
} /* readpack */
...
writepack() /* write pack */
{
	...
	fp1=fopen(outputfile , "w");
        ...
        for (i=1;i<=count;i++)
	{
		fprintf(fp1," % .6f % .6f  ",pR_ptr[i].center.re,
			pR_ptr[i].center.im);
		fprintf(fp1,"\n");
        }
        fclose(fp1);
...
float s,aim;
{
	...
	bestrad=s;
	if (aim<0.00001 && packK[i].bdry_flag) return (-.2);
	if (s<=0 && aim>0.00001) bestrad=.5;
	...
	while ( (bestcurv>(aim+TOLER) || bestcurv<(aim-TOLER))
		&& (n < iterates))
	 {
		...
		if (tryrad<bestrad*0.5) tryrad=bestrad*0.5;
		if (tryrad>(1+bestrad)*0.5) tryrad=(1+bestrad)*0.5;
